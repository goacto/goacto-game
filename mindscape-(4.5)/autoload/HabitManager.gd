...
func _ready() -> void:
	# IMPORTANT: Do NOT initialize preset habits here
	# Habits should only be created when:
	# 1. Console is placed (calls initialize_starter_habits)
	# 2. Loading a save that already has habits
	print("[HabitManager] _ready called, habits count BEFORE any init: ", habits.size())
	_check_daily_reset()
	_setup_reminder_system()
	print("[HabitManager] Initialized with ", habits.size(), " habits (should be 0 for new game)")


## Initialize preset habits when console is placed (called externally)
func initialize_starter_habits() -> void:
	# Only initialize if no habits exist yet
	if habits.size() > 0:
		print("[HabitManager] Habits already exist, skipping initialization")
		return
	_initialize_preset_habits()
	print("[HabitManager] Starter habits initialized")
...
func complete_habit(habit_id: String) -> Dictionary:
	...
	if accessibility:
		accessibility.announce_habit_complete(habit.name, habit.streak)
