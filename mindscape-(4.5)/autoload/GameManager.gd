...
func _ready() -> void:
	...
	# Create version overlay (persistent across all scenes)
...
func _create_version_overlay() -> void:
	...
	version_overlay.layer = 90  # Below transitions but above game
	add_child(version_overlay)

	var container = Control.new()
	container.name = "VersionContainer"
	container.set_anchors_preset(Control.PRESET_FULL_RECT)
	container.mouse_filter = Control.MOUSE_FILTER_IGNORE
	version_overlay.add_child(container)
	...
	label.add_theme_color_override("font_color", Color(0.5, 0.5, 0.55, 0.6))
...
func _check_aspect_levelup(aspect_key: String) -> void:
	...
	if aspect.experience >= exp_needed:
		...
		if accessibility:
			accessibility.announce(aspect.name + " leveled up to level " + str(aspect.level))
...
func complete_focus_session_with_multiplier(minutes: int, habit_domain: String, xp_multiplier: float) -> void:
	...
	var accessibility = get_node_or_null("/root/AccessibilityManager")
	if accessibility:
		accessibility.announce_focus_session(habit_domain, minutes, "end")
...
func has_seen_tutorial(tutorial_id: String) -> bool:
	var seen = player_data.get("seen_tutorials", [])
	return tutorial_id in seen


## Mark a tutorial as seen
func mark_tutorial_seen(tutorial_id: String) -> void:
	if not player_data.has("seen_tutorials"):
		player_data["seen_tutorials"] = []
