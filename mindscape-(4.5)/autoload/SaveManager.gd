...
func _get_default_settings() -> Dictionary:
	return {
		...
		# Cutscene options
		"auto_play_cutscenes": false,  # Auto-advance dialogue after typing completes
		"auto_play_delay": 1.5  # Seconds to wait before auto-advancing
	}
...
func rename_slot(slot: int, new_name: String) -> bool:
	...
	if not has_slot_save(slot):
		push_error("[SaveManager] No save exists in slot: ", slot)
		return false

	var path = _get_slot_path(slot)
	var file = FileAccess.open(path, FileAccess.READ)
	if not file:
		push_error("[SaveManager] Could not open slot for renaming: ", slot)
		return false
	...
	var json = JSON.new()
	if json.parse(json_string) != OK:
		push_error("[SaveManager] Could not parse save data for renaming")
		return false

	var data = json.get_data()
	data["slot_name"] = new_name.strip_edges()
	if data["slot_name"] == "":
		data["slot_name"] = "Slot " + str(slot)

	# Write back
	var write_file = FileAccess.open(path, FileAccess.WRITE)
	if not write_file:
		push_error("[SaveManager] Could not write renamed save")
		return false

	write_file.store_string(JSON.stringify(data, "\t"))
	write_file.close()

	print("[SaveManager] Renamed slot ", slot, " to '", data["slot_name"], "'")
	return true
