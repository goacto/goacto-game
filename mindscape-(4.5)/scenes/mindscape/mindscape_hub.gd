...
func _ready() -> void:
	...
	if accessibility:
		var evo_level = GameManager.player_data.get("world_evolution_level", 0.0)
		accessibility.announce_location("Mindscape Hub", "Evolution level %d percent" % int(evo_level))
...
func _create_garden_patches() -> void:
	...
	# Garden patch positions around the hub - pushed to outer edges to avoid overlaps
	var patch_configs = [
		{"name": "Mind", "pos": Vector2(-450, -120), "base_color": Color(0.3, 0.5, 0.8)},
		{"name": "Body", "pos": Vector2(450, -120), "base_color": Color(0.8, 0.4, 0.3)},
		{"name": "Soul", "pos": Vector2(-500, 80), "base_color": Color(0.6, 0.4, 0.8)},
		{"name": "Social", "pos": Vector2(500, 80), "base_color": Color(0.4, 0.7, 0.5)},
		{"name": "Career", "pos": Vector2(-400, 250), "base_color": Color(0.7, 0.6, 0.3)},
		{"name": "Wealth", "pos": Vector2(400, 250), "base_color": Color(0.8, 0.7, 0.2)}
	]
...
func _setup_achievement_pedestals() -> void:
	...
	# Three pedestal positions - horizontal row higher up between exit portal and north gate
	var positions = [
		Vector2(-50, -200),
		Vector2(0, -210),
		Vector2(50, -200)
	...
	label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	label.position = Vector2(-50, -240)  # Above the center pedestal
	label.size = Vector2(100, 15)
...
func _update_camera() -> void:
	...
	isometric_base.position = target.round()
...
func _create_progress_indicators() -> void:
	...
	# === EVOLUTION RING ===
	# A subtle ring around the platform that fills based on evolution progress
	evolution_ring = Node2D.new()
	evolution_ring.name = "EvolutionRing"
	evolution_ring.position = Vector2(0, 0)

	var ring_radius = 140  # Smaller, less obtrusive
	var ring_thickness = 4  # Thinner
	var segment_count = 16  # Fewer segments for cleaner look
	...
	# Evolution level indicator - smaller, positioned near the ring
	var evo_label = Label.new()
	evo_label.name = "EvoLabel"
	evo_label.text = "Evo " + str(evolution_level)
	evo_label.add_theme_font_size_override("font_size", 10)
	evo_label.add_theme_color_override("font_color", Color(ring_color.r, ring_color.g, ring_color.b, 0.7))
	evo_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	evo_label.position = Vector2(-20, -160)  # Adjusted for smaller ring
...
func _animate_hub_environment(delta: float) -> void:
	...
	# Animate evolution ring segments (very subtle glow pulse)
	for seg in evolution_segments:
		if is_instance_valid(seg.polygon):
			var pulse = 0.9 + sin(hub_env_time * 1.0 + seg.index * 0.2) * 0.1  # Much subtler pulse
			var color = seg.base_color
			seg.polygon.color = Color(color.r, color.g, color.b, color.a * pulse * 0.7)  # More transparent
...
func _check_daily_login() -> void:
	...
	if result.is_new_day:
		...
		if result.reward and not result.reward.is_empty():
			...
			# Wait for onboarding to complete before showing daily reward
			while is_onboarding_active and is_inside_tree():
				await get_tree().create_timer(0.5).timeout

			if not is_inside_tree():
				return

			# Additional delay after onboarding finishes
			if GameManager.player_data.get("has_completed_onboarding", false):
				await get_tree().create_timer(0.5).timeout
