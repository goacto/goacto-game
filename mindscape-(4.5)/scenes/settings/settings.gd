...
func _ready() -> void:
	...
	# Build screen reader section
	_build_screen_reader_section()

	# Build auto-play section
	_build_auto_play_section()
...
# Screen reader UI (built dynamically)
var screen_reader_toggle: CheckButton = null
var speech_rate_slider: HSlider = null
var speech_rate_value: Label = null
var test_tts_button: Button = null

# Auto-play UI (built dynamically)
var auto_play_toggle: CheckButton = null
var auto_play_delay_slider: HSlider = null
var auto_play_delay_value: Label = null
...
func _build_screen_reader_section() -> void:
	var vbox = $Panel/Margin/ScrollContainer/VBox
	var accessibility_section = $Panel/Margin/ScrollContainer/VBox/AccessibilitySection
	if not vbox or not accessibility_section:
		return

	# Get the index to insert after AccessibilitySection
	var insert_idx = accessibility_section.get_index() + 1

	# Create screen reader section
	var section = VBoxContainer.new()
	section.name = "ScreenReaderSection"
	...
	# Section title
	var title = Label.new()
	title.text = "Screen Reader (TTS)"
	title.add_theme_font_size_override("font_size", 16)
	title.add_theme_color_override("font_color", Color(0.6, 0.55, 0.7, 1))
	section.add_child(title)

	# Check TTS availability
	var tts_available = DisplayServer.tts_get_voices().size() > 0

	if not tts_available:
		var warning = Label.new()
		warning.text = "Text-to-speech not available on this device"
		warning.add_theme_font_size_override("font_size", 12)
		warning.add_theme_color_override("font_color", Color(0.6, 0.5, 0.5))
		section.add_child(warning)
		return

	# Screen Reader Enable toggle row
	var enable_row = HBoxContainer.new()
	enable_row.add_theme_constant_override("separation", 10)

	var enable_label = Label.new()
	enable_label.text = "Enable Screen Reader"
	enable_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	enable_row.add_child(enable_label)

	screen_reader_toggle = CheckButton.new()
	screen_reader_toggle.button_pressed = _get_screen_reader_enabled()
	screen_reader_toggle.toggled.connect(_on_screen_reader_toggled)
	enable_row.add_child(screen_reader_toggle)
	...
	rate_label.text = "Speech Rate"
	rate_label.custom_minimum_size = Vector2(120, 0)
	rate_row.add_child(rate_label)

	speech_rate_slider = HSlider.new()
	speech_rate_slider.min_value = 50
	speech_rate_slider.max_value = 200
	speech_rate_slider.step = 10
	speech_rate_slider.value = _get_speech_rate() * 100
	speech_rate_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	speech_rate_slider.custom_minimum_size = Vector2(150, 0)
	speech_rate_slider.value_changed.connect(_on_speech_rate_changed)
	rate_row.add_child(speech_rate_slider)

	speech_rate_value = Label.new()
	speech_rate_value.text = "%d%%" % int(speech_rate_slider.value)
	speech_rate_value.custom_minimum_size = Vector2(45, 0)
	rate_row.add_child(speech_rate_value)
	...
	test_label.text = "Test Speech"
	test_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	test_row.add_child(test_label)

	test_tts_button = Button.new()
	test_tts_button.text = "Test"
	test_tts_button.custom_minimum_size = Vector2(80, 30)
	test_tts_button.pressed.connect(_test_screen_reader)
	test_row.add_child(test_tts_button)

	section.add_child(test_row)

	# Audio sound for button
	var audio = get_node_or_null("/root/AudioManager")
	if audio:
		test_tts_button.pressed.connect(audio.play_ui_click)


func _get_screen_reader_enabled() -> bool:
	var accessibility = get_node_or_null("/root/AccessibilityManager")
	if accessibility:
		return accessibility.screen_reader_enabled
	return false


func _get_speech_rate() -> float:
	var accessibility = get_node_or_null("/root/AccessibilityManager")
	if accessibility:
		return accessibility.speech_rate
	return 1.0
...
func _on_screen_reader_toggled(pressed: bool) -> void:
	var accessibility = get_node_or_null("/root/AccessibilityManager")
	if accessibility:
		accessibility.set_screen_reader_enabled(pressed)
	print("[Settings] Screen reader: ", pressed)


func _on_speech_rate_changed(value: float) -> void:
	var rate = value / 100.0
	var accessibility = get_node_or_null("/root/AccessibilityManager")
	if accessibility:
		accessibility.set_speech_rate(rate)
	if speech_rate_value:
		speech_rate_value.text = "%d%%" % int(value)


func _test_screen_reader() -> void:
	var accessibility = get_node_or_null("/root/AccessibilityManager")
	if not accessibility:
		print("[Settings] AccessibilityManager not available")
		return

	# Temporarily enable for test if not enabled
	var was_enabled = accessibility.screen_reader_enabled
	if not was_enabled:
		accessibility.screen_reader_enabled = true

	accessibility.announce("This is a test of the screen reader. Welcome to GOACTO Mindscape.")

	# Restore previous state after speaking
	if not was_enabled:
		await get_tree().create_timer(3.0).timeout
		accessibility.screen_reader_enabled = was_enabled
...
func _build_auto_play_section() -> void:
	var vbox = $Panel/Margin/ScrollContainer/VBox
	var accessibility_section = $Panel/Margin/ScrollContainer/VBox/AccessibilitySection
	if not vbox or not accessibility_section:
		return

	# Get the index to insert after ScreenReaderSection (which is after AccessibilitySection)
	var insert_idx = accessibility_section.get_index() + 2  # +1 for ScreenReaderSection, +1 for after it

	# Create auto-play section
	var section = VBoxContainer.new()
	section.name = "AutoPlaySection"
	section.add_theme_constant_override("separation", 8)
	vbox.add_child(section)
	vbox.move_child(section, insert_idx)
	...
	# Section title
	var title = Label.new()
	title.text = "Cutscene Auto-Play"
	title.add_theme_font_size_override("font_size", 16)
	title.add_theme_color_override("font_color", Color(0.6, 0.55, 0.7, 1))
	section.add_child(title)

	# Auto-play enable toggle row
	var enable_row = HBoxContainer.new()
	enable_row.add_theme_constant_override("separation", 10)

	var enable_label = Label.new()
	enable_label.text = "Auto-advance dialogue"
	enable_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	enable_row.add_child(enable_label)

	auto_play_toggle = CheckButton.new()
	auto_play_toggle.button_pressed = _get_auto_play_enabled()
	auto_play_toggle.toggled.connect(_on_auto_play_toggled)
	enable_row.add_child(auto_play_toggle)
	...
	delay_label.text = "Delay before advancing"
	delay_label.custom_minimum_size = Vector2(160, 0)
	delay_row.add_child(delay_label)

	auto_play_delay_slider = HSlider.new()
	auto_play_delay_slider.min_value = 50
	auto_play_delay_slider.max_value = 400
	auto_play_delay_slider.step = 25
	auto_play_delay_slider.value = _get_auto_play_delay() * 100
	auto_play_delay_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	auto_play_delay_slider.custom_minimum_size = Vector2(150, 0)
	auto_play_delay_slider.value_changed.connect(_on_auto_play_delay_changed)
	delay_row.add_child(auto_play_delay_slider)

	auto_play_delay_value = Label.new()
	auto_play_delay_value.text = "%.1fs" % _get_auto_play_delay()
	auto_play_delay_value.custom_minimum_size = Vector2(40, 0)
	delay_row.add_child(auto_play_delay_value)

	section.add_child(delay_row)

	# Description
	var desc = Label.new()
	desc.text = "When enabled, cutscenes advance automatically after text appears"
	desc.add_theme_font_size_override("font_size", 11)
	desc.add_theme_color_override("font_color", Color(0.5, 0.5, 0.55))
	section.add_child(desc)


func _get_auto_play_enabled() -> bool:
	var settings = SaveManager.load_settings()
	return settings.get("auto_play_cutscenes", false)


func _get_auto_play_delay() -> float:
	var settings = SaveManager.load_settings()
	return settings.get("auto_play_delay", 1.5)


func _on_auto_play_toggled(pressed: bool) -> void:
	var settings = SaveManager.load_settings()
	settings["auto_play_cutscenes"] = pressed
	SaveManager.save_settings(settings)
	print("[Settings] Auto-play cutscenes: ", pressed)


func _on_auto_play_delay_changed(value: float) -> void:
	var delay = value / 100.0
	var settings = SaveManager.load_settings()
	settings["auto_play_delay"] = delay
	SaveManager.save_settings(settings)
	if auto_play_delay_value:
		auto_play_delay_value.text = "%.1fs" % delay
