...
@onready var auto_countdown: Label = $DialoguePanel/AutoCountdown
...
# Auto-play settings (default OFF for each cutscene, user must enable)
var auto_play_enabled: bool = false
var auto_play_delay: float = 2.5  # Increased buffer between transitions
...
func _load_auto_play_settings() -> void:
	# Always start with auto-play OFF for each cutscene
	# User must manually enable it each time
	auto_play_enabled = false
	auto_play_delay = 2.5  # Comfortable reading buffer
...
func _setup_ui_sounds() -> void:
	...
	for btn in [skip_button, back_button, auto_button]:
...
func _on_auto_toggle() -> void:
	...
	_update_auto_button_visual()
	# If auto-play just enabled and we're waiting for input, start the timer
	if auto_play_enabled and not is_typing and continue_hint.visible:
...
func _update_auto_button_visual() -> void:
	...
	if auto_play_enabled:
		...
		auto_button.add_theme_color_override("font_color", Color(0.4, 0.8, 0.5, 1))
	else:
		auto_button.text = "Auto"
		auto_button.add_theme_color_override("font_color", Color(0.5, 0.5, 0.6, 1))
...
func _process(delta: float) -> void:
	...
	if auto_play_waiting and auto_play_enabled:
		...
		# Update countdown label (positioned in top-right, doesn't affect layout)
		var remaining = max(0, auto_play_delay - auto_play_timer)
		if auto_countdown:
			auto_countdown.text = "Auto: %.1fs" % remaining
			auto_countdown.visible = true
		if auto_play_timer >= auto_play_delay:
			auto_play_waiting = false
			auto_play_timer = 0.0
			if auto_countdown:
				auto_countdown.visible = false
...
func _on_typing_completed() -> void:
	if auto_play_enabled:
		...
		auto_play_waiting = true
		auto_play_timer = 0.0
		# Show countdown in top-right (doesn't shift layout)
		if auto_countdown:
			auto_countdown.text = "Auto: %.1fs" % auto_play_delay
			auto_countdown.visible = true
		continue_hint.text = "[Tap to skip]"
		continue_hint.visible = true
	else:
		continue_hint.text = "[Space/Tap to continue]"
		continue_hint.visible = true
		if auto_countdown:
			auto_countdown.visible = false
