...
var object_positions: Dictionary = {
	...
	"ShipMemento": Vector2(300, 40),
}

# Visual node for collectible memento
var memento_visual: Node2D = null
...
func _ready() -> void:
	...
	# Create collectible memento visual (if not already collected)
	_setup_collectible_memento()
...
func _setup_collectible_memento() -> void:
	## Create visual for ship memento if not already collected
	var collected = GameManager.player_data.get("collected_decor", [])
	if "decor_ship_memento" in collected:
		# Already collected, remove from interactables
		object_positions.erase("ShipMemento")
		return

	# Create the memento visual near the bookshelf
	memento_visual = Node2D.new()
	memento_visual.name = "ShipMemento"
	memento_visual.position = object_positions.get("ShipMemento", Vector2(300, 40))
	...
	for i in range(16):
		...
		ring_points.append(Vector2(cos(angle) * 28, sin(angle) * 20))
	glow_ring.polygon = ring_points
	glow_ring.color = Color(1.0, 0.85, 0.4, 0.15)
	glow_ring.z_index = -1
	memento_visual.add_child(glow_ring)
	...
	for i in range(12):
		...
		glow_points.append(Vector2(cos(angle) * 20, sin(angle) * 14))
	glow.polygon = glow_points
	glow.color = Color(1.0, 0.9, 0.5, 0.25)
	glow.z_index = -1
	memento_visual.add_child(glow)

	# Pedestal base
	var pedestal = Polygon2D.new()
	pedestal.polygon = PackedVector2Array([
		Vector2(-18, 8), Vector2(18, 8), Vector2(14, 14), Vector2(-14, 14)
	])
	pedestal.color = Color(0.4, 0.35, 0.3, 0.9)
	memento_visual.add_child(pedestal)

	# Memento base - small decorative ship model
	var base = Polygon2D.new()
	base.polygon = PackedVector2Array([
		Vector2(-15, 5), Vector2(15, 5), Vector2(12, -2), Vector2(-12, -2)
	])
	base.color = Color(0.55, 0.5, 0.4, 0.95)
	memento_visual.add_child(base)

	# Ship hull shape
	var hull = Polygon2D.new()
	hull.polygon = PackedVector2Array([
		Vector2(-12, 0), Vector2(-10, -8), Vector2(-5, -12),
		Vector2(8, -12), Vector2(12, -8), Vector2(10, 0)
	])
	hull.color = Color(0.65, 0.6, 0.5, 0.95)
	memento_visual.add_child(hull)

	# Ship details - window
	var window = Polygon2D.new()
	window.polygon = PackedVector2Array([
		Vector2(-4, -10), Vector2(2, -10), Vector2(2, -7), Vector2(-4, -7)
	])
	window.color = Color(0.4, 0.7, 0.9, 0.8)
	memento_visual.add_child(window)

	# Ship fin
	var fin = Polygon2D.new()
	fin.polygon = PackedVector2Array([
		Vector2(-8, -8), Vector2(-6, -15), Vector2(-4, -8)
	])
	fin.color = Color(0.7, 0.65, 0.55, 0.9)
	memento_visual.add_child(fin)
	...
	for i in range(4):
		...
		var angle = (i / 4.0) * TAU
		sparkle.position = Vector2(cos(angle) * 22, sin(angle) * 16 - 4)
		sparkle.color = Color(1.0, 0.95, 0.7, 0.6)
		memento_visual.add_child(sparkle)

	# "NEW" indicator arrow bouncing above
	var arrow = Polygon2D.new()
	arrow.name = "Arrow"
	arrow.polygon = PackedVector2Array([
		Vector2(0, -25), Vector2(6, -18), Vector2(2, -18),
		Vector2(2, -12), Vector2(-2, -12), Vector2(-2, -18), Vector2(-6, -18)
	])
	arrow.color = Color(0.3, 0.9, 0.4, 0.9)
	memento_visual.add_child(arrow)

	isometric_base.add_child(memento_visual)


func _pickup_ship_memento() -> void:
	## Player picks up the ship memento
	var collected = GameManager.player_data.get("collected_decor", [])
	if "decor_ship_memento" in collected:
		_show_dialogue("Already Collected", "You already have this memento in your collection.")
		return

	# Add to collected decorations
	collected.append("decor_ship_memento")
	GameManager.player_data["collected_decor"] = collected
	SaveManager.save_game()

	# Remove the visual
	if memento_visual:
		memento_visual.queue_free()
		memento_visual = null
	...
	# Show pickup message
	_show_dialogue(
		"Ship Memento Collected!",
		"You picked up a small memento from the Stellar Wanderer.\n\n" +
		"*A piece of home to decorate your room*\n\n" +
		"Visit your bedroom to place this decoration!"
	)
...
func _animate_tv(delta: float) -> void:
	...
	# Animate memento collectible if present
	_animate_memento()


func _animate_memento() -> void:
	if not memento_visual:
		return

	# Pulsing glow ring
	var glow_ring = memento_visual.get_node_or_null("GlowRing")
	if glow_ring:
		var pulse = 0.1 + sin(animation_time * 2.5) * 0.1
		glow_ring.color.a = pulse
		glow_ring.scale = Vector2(1.0 + sin(animation_time * 2.0) * 0.1, 1.0 + sin(animation_time * 2.0) * 0.1)

	# Secondary glow
	var glow = memento_visual.get_node_or_null("Glow")
	if glow:
		var pulse = 0.2 + sin(animation_time * 3.0 + 0.5) * 0.15
		glow.color.a = pulse

	# Rotating/twinkling sparkles
	for i in range(4):
		var sparkle = memento_visual.get_node_or_null("Sparkle" + str(i))
		if sparkle:
			var base_angle = (i / 4.0) * TAU
			var orbit_angle = base_angle + animation_time * 1.5
			sparkle.position = Vector2(cos(orbit_angle) * 24, sin(orbit_angle) * 18 - 4)
			sparkle.rotation = animation_time * 2.0
			sparkle.color.a = 0.4 + sin(animation_time * 4.0 + i) * 0.4

	# Bouncing arrow indicator
	var arrow = memento_visual.get_node_or_null("Arrow")
	if arrow:
		arrow.position.y = -25 + sin(animation_time * 4.0) * 4
