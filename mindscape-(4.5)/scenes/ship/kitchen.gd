...
# Interactive objects in kitchen (simplified - essential items only)
const INTERACTIVE_OBJECTS = {
	...
	"BalconyDoor": {
		...
		"prompt": "Press SPACE to go outside",
		"action": "go_balcony"
	},
	"Mom": {
		"name": "Dr. Lumina",
	...
	"FoodUnit": {
		...
		"prompt": "Press SPACE to use",
		"action": "use_food_synth"
	}
}

# Object positions for proximity detection (simplified)
...
func _ready() -> void:
	...
	# Show intro hint after fade completes (only for first visit, and only once per entry)
	await get_tree().create_timer(0.6).timeout
	if not CampaignManager.has_seen_cutscene("intro_part2") and not GameManager.has_seen_tutorial("kitchen_intro_hint"):
		# First time - hasn't used mindscape yet, and hasn't seen this hint yet
		GameManager.mark_tutorial_seen("kitchen_intro_hint")
	...
	if accessibility:
		accessibility.announce_location("Kitchen", "Family dining area on the ship")
...
func _interact_with_object(object_id: String) -> void:
	...
	match action:
		...
		"look_window":
			_show_space_view()
		"use_food_synth":
			_show_dialogue("Food Synthesizer", "The synthesizer hums softly, ready to materialize any meal.\n\nA small screen displays today's menu options.")
